class n extends HTMLElement{constructor(){super();const e=`
    ...
    `;this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
      ...
      ${e}
    `,this.button=this.shadowRoot.querySelector('[aria-label="Page settings"]');const i=window.matchMedia("(prefers-color-scheme: dark)").matches,t=window.matchMedia("(prefers-contrast: more)").matches;this.shadowRoot.querySelector(`input[value=${i?"dark":"light"}${t?"-hc":""}`).checked=!0,document.body.dataset.iuiTheme=i?"dark":"light",document.body.dataset.iuiContrast=t?"high":void 0,document.body.classList.toggle("iui-root",!0)}changeTheme=({target:{value:e}})=>{const a=document.body,i=document.documentElement,t=e.endsWith("-hc"),o=t?e.split("-")[0]:e;a.dataset.iuiTheme=o,i.dataset.colorScheme=o,a.dataset.iuiContrast=t?"high":void 0,this.shadowRoot.querySelector("#theme-color-scheme").innerHTML=`
    :host {
      color-scheme: ${o.includes("light")?"light":"dark"};
      }
      `};changeBackground=({target:{value:e}})=>{e==="bg1"?document.body.style.backgroundColor="var(--iui-color-background)":document.body.removeAttribute("style")};changeBridge=({target:{checked:e}})=>{document.body.dataset.iuiBridge=e?"true":"false"};connectedCallback(){this.shadowRoot.querySelectorAll('input[name="theme"]').forEach(e=>{e.addEventListener("change",this.changeTheme)}),this.shadowRoot.querySelectorAll('input[name="background"]').forEach(e=>{e.addEventListener("change",this.changeBackground)}),this.shadowRoot.querySelector('input[name="bridge"]').addEventListener("change",this.changeBridge)}disconnectedCallback(){this.shadowRoot.querySelectorAll('input[name="theme"]').forEach(e=>{e.removeEventListener("change",this.changeTheme)}),this.shadowRoot.querySelectorAll('input[name="background"]').forEach(e=>{e.removeEventListener("change",this.changeBackground)}),this.shadowRoot.querySelector('input[name="bridge"]').removeEventListener("change",this.changeBridge)}}customElements.define("theme-button",n);
