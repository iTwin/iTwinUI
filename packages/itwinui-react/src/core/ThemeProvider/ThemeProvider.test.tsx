...
it('should dispatch custom events when theme changes', () => {
  const onThemeChange = jest.fn();

  const Wrapper = (props: Record<string, unknown>) => (
    <ThemeProvider
      ref={(el) => {
        el?.addEventListener('iui:themechange', onThemeChange);
      }}
      {...props}
    >
      hello
  ...
  rerender(<Wrapper theme='dark' themeOptions={{ highContrast: true }} />);
  expect(onThemeChange).toHaveBeenCalledWith(
    expect.objectContaining({ detail: { theme: 'dark', highContrast: true } }),
  );

  rerender(<Wrapper theme='light' themeOptions={{ highContrast: false }} />);
  expect(onThemeChange).toHaveBeenCalledWith(
    expect.objectContaining({
      detail: { theme: 'light', highContrast: false },
    }),
  );
});

it('should listen to custom themechange events', () => {
  const Wrapper = (props: Record<string, unknown>) => (
    <div
      data-iui-theme={props.theme}
      ref={(el) => {
        el?.dispatchEvent(
          new CustomEvent('iui:themechange', { detail: props }),
        );
      }}
    >
      <ThemeProvider theme='inherit' data-root>
        hello
      </ThemeProvider>
    </div>
  );

  const { rerender } = render(<Wrapper theme='light' highContrast={false} />);
  const root = document.querySelector('[data-root]') as HTMLElement;

  expect(root).toHaveAttribute('data-iui-theme', 'light');
  expect(root).toHaveAttribute('data-iui-contrast', 'default');

  rerender(<Wrapper theme='dark' highContrast={false} />);
  expect(root).toHaveAttribute('data-iui-theme', 'dark');
  expect(root).toHaveAttribute('data-iui-contrast', 'default');

  rerender(<Wrapper theme='light' highContrast={true} />);
  expect(root).toHaveAttribute('data-iui-theme', 'light');
  expect(root).toHaveAttribute('data-iui-contrast', 'high');

  rerender(<Wrapper theme='dark' highContrast={true} />);
  expect(root).toHaveAttribute('data-iui-theme', 'dark');
  expect(root).toHaveAttribute('data-iui-contrast', 'high');
});
