...
`LinkAction` is used internally in [`Tile`](tile) and [`ListItem`](list) components, and is also available for use in external components.
...
If your container includes other elements that should be clickable, you can set `position: relative` on them or give them a higher z-index. This will make them appear on top of the `LinkBox` and receive the click event instead of the primary `LinkAction`.
