...
The statuses for both `ProgressLinear` and `ProgressRadial` are used to convey the outcome of a process. The desired UX flow is to show a default progress indicator during the process, and then update the status to reflect whether the process was successful, encountered an error, or requires attention. Three available statuses are:
