...
By default, each step circle displays the step number. However, end users can customize the icon or content shown for completed steps in the `Stepper` component by passing the `stepCircleRenderer` property in each step item. This property is a function that takes the step index and returns the custom content to represent step completion.
