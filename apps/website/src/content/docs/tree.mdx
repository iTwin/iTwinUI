...
Each object in the `data` array can take information under the `subItems` array as its children, which allows for nested structures.
...
To handle expandable subnodes, a state variable is needed to track each node and its expansion state.

When a user interacts with a node, the `onExpand` function in each `TreeNode` can be used to update the node's expansion state accordingly.
...
To correctly display subnodes, the `getNode` function must return the node state with subnode information before rendering each node. This includes:
