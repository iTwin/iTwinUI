...
The `Text` component includes a `variant` prop that allows you to adjust the text size. Available options range from `"headline"` to `"smaller"`, with `"body"` set as the default size. The `as` prop is also provided, which allows you to specify the HTML element to render, defaulting to a `<span>`.
