...
The `Text` component includes a `variant` prop that allows you to adjust the text size. Available options range from `"headline"` to `"smaller"`, with `"body"` set as the default size. The `as` prop allows you to choose which HTML element the `Text` component should render, with `<span>` being the default.
