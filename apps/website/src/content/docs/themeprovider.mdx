...
To make the current iTwinUI theme available globally, enable `themeOptions.synchronizeThemeToRoot`. This propagates the relevant attributes to the `<body>` element, making iTwinUI CSS variables available everywhere on the page. This can be useful in cases where iTwinUI variables or components need to be rendered in a third-party portal located outside the `ThemeProvider`'s DOM subtrees.
