...
By default, `ThemeProvider` scopes all styles to the DOM subtree that it renders (including its own [portal container](#portals)).

To make the current iTwinUI theme available globally, enable `themeOptions.synchronizeThemeToRoot`. This propagates the relevant data attributes to the `<body>` element, making iTwinUI CSS variables available everywhere on the page. This can be useful in cases where iTwinUI variables or components need to be rendered in a third-party portal located outside the `ThemeProvider`'s DOM subtrees.
...
This option defaults to `true` when [future flags](future-flags#synchronizethemetoroot) are enabled.
