...
`ExpandableBlock` with customizable `Title`, `Caption`, `Content` and `EndIcon` subcomponents.
...
The `ExpandableBlock.Content` subcomponent is used for the content shown in the block's body when fully expanded.
