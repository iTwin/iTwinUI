...
To enable sorting in the `Table`, set the `isSortable` prop to `true`. This will allow the columns to display an arrow when hovered, indicating whether the column is sorted in ascending or descending order.

<LiveExample src='Table.sorting.jsx'>
  <AllExamples.TableSortingExample client:load />
</LiveExample>

By default, columns are sorted in ascending order first when the arrow is clicked. To prioritize descending sorting instead, set the `sortDescFirst` property to `true` in the column object.
