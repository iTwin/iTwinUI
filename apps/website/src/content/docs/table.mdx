...
For tables with large datasets, the `enableVirtualization` prop can be set to `true` to enhance performance by only rendering the rows that are currently in the viewport.
...
There are two available selection modes, `"single"` and `"multi"`.
