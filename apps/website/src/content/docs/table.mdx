...
**Note**: The `Table` component is built based on `react-table` v7. For more information about `react-table`, see their [docs](https://react-table.tanstack.com/docs/api/overview).
...
By default, row ids are based on depth and row indexes (e.g. `3-0-1` for the fourth row's first subrow's second row). Use `getRowId` for custom ids (e.g. from data source) to have with 1-1 match between the data and the row. E.g. useful when want to select rows in `initialState.selectedRowIds`.
...
**Note**: This feature is still considered _experimental_ and so may have some performance issues, bugs, future changes, etc. If If virtualization does not work, [pagination](#pagination) is the next best alternative for large datasets.
...
To enable pagination, pass the `paginatorRenderer` prop that takes `TablePaginatorRendererProps` as an argument and returns a pagination component (recommended: `<TablePaginator>`). It is highly recommended for `paginatorRenderer` to return `<TablePaginator>`. Passing/Spreading `props` to `TablePaginator` handles all state management and is enough for basic use-cases.
...
```jsx
const paginator = React.useCallback((props) => <TablePaginator {...props} />, [pageSizeList]);
...
The `ActionColumn` is a column that should be placed at the end to perform actions on each row. If `columnManager` is set to `true`, a column manager button is disabled in the action column's header cell to help toggle the visibility of certain columns.
...
Sort arrows (`↑` for ascending or ↓ for descending) are displayed in the header cells according to the following:
