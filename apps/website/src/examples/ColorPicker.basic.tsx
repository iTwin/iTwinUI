...
import { ColorPalette, ColorPicker, ColorValue, ColorSwatch } from '@itwin/itwinui-react';
...
export default () => {
  ...
  const onColorChanged = (color: ColorValue) => {
    ...
    const index = ColorsList.findIndex((swatch) => swatch.color === hexString.toLowerCase());
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', flexDirection: 'row', gap: 10 }}>
        <ColorSwatch style={{ pointerEvents: 'none', margin: 0 }} color={activeColor.color} />
        <span>{colorName}</span>
      </div>
      <ColorPicker selectedColor={activeColor.color} onChangeComplete={onColorChanged}>
