...
The property `overflowButton` allows you to show a custom button when truncation is necessary. You can add a dropdown to show all of the hidden breadcrumbs, as in the example below.

<LiveExample src='Breadcrumbs.customOverflowDropdown.tsx'>
  <AllExamples.BreadcrumbsCustomOverflowDropdownExample client:load />
</LiveExample>

Another option is to make the overflow button act as a 'back button' that takes users to the last hidden breadcrumb.

<LiveExample src='Breadcrumbs.customOverflowBackButton.tsx'>
  <AllExamples.BreadcrumbsCustomOverflowBackButtonExample client:load />
</LiveExample>
