...
Add only custom elements by passing `customElements` prop to Footer component. It expects an array of objects each describing a custom link for the footer. They are all the elements that we pass. Set the `title` and `url` property for each defined custom element.
