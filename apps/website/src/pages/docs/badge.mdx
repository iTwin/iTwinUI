...
exampleCodeMain: Badge.main.tsx
exampleCodeSoft: Badge.soft.tsx
exampleCodeStatuses: Badge.statuses.tsx
thumbnail: Badge
group: core
---
...
Badges are colorful rectangles with rounded corners, with a label in white, all caps font on top. Badges are available in one size only; the length is flexible to accommodate different labels, but the height or font size should not be increased. There are two different categories of colors to be used by badges:

### Soft backgrounds

<LiveExample src={frontmatter.exampleCodeSoft} />
