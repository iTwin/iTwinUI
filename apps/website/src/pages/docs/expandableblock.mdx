...
The expandable block can be smaller by using the `size` prop
...
The borderless expandable block is useful in tighter areas where using a default expandable block would visibly cause too many blocks. This borderless expandable block is achieved by using the `styleType` prop.
