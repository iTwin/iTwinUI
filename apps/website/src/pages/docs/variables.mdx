...
We strongly encourage the use iTwinUI's CSS variables rather than hardcoding values. Many of the color values change depending on the theme or contrast settings. Because of this, we purposefully do not expose the variables' values on this site. However, if you need a particular variable's value, you can extrapolate it from [the raw CSS file](https://unpkg.com/browse/@itwin/itwinui-variables/index.css).
...
If you need to animate anything, use one of the supplied duration variables. The duration scale decreases from 0 to 3, with `--iui-duration-0` having no duration. For most hover or click interactions, we encourage using the fastest speed.
