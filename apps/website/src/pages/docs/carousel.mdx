...
located below the slides, consisting of ["dots"](#dots-list) and ["previous"/"next"](#navigation) buttons, used for changing slides.
...
By default, the carousel maintain their own state. it can be controlled externally by using the properties `activeSlideIndex` and `onSlideChange`.
