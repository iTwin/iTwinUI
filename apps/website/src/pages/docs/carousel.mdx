...
The Carousel component consists of a set of slides, normally displayed one at a time. A navigation section is located below the slides, consisting of "dots" ([Carousel.DotsList](#carouseldotslist)) and "previous"/"next" ([Carousel.Navigation](#carouselnavigation)) buttons, used for changing slides.

<PropsTable path={frontmatter.propsPath} />

## Usage

By default, the usage of all subcomponents is not mandatory. We have taken extra care to reduce verbosity while designing the Carousel API.
...
**Important**: The `Carousel.DotsList` component must always be present before the slides in DOM order. In addition, this subcomponent is optional and can also be used standalone.
