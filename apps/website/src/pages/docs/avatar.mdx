...
Different properties can be passed to the `<Avatar>` component to customize it according to the user's needs.

- `title` prop can be used to specify the name or title associated with the avatar.
- `abbreviation` prop allows users to provide a short abbreviation or initials for the avatar.
- `image` prop can be passed to set a custom image source or icon that will be displayed within the avatar.

### Sizes

Avatars can be displayed in various sizes, including `small`, `medium`, `large`, and `x-large`. These options offer users the flexibility to choose the most appropriate avatar size.

<LiveExample src='Avatar.sizes.tsx'>
  <AllExamples.AvatarSizesExample client:load />
</LiveExample>

By making use of the `size` prop and selecting one of the predefined values mentioned above, you can easily set the size of the `Avatar` component.

### Status indicators

Status indicators are indicators that display in the bottom right of the avatar. Statuses can be set using the `status` prop. The available statuses are: `online`, `away`, `busy` and `offline`.
...
An avatar can be customized to show initials, icons, and pictures.

<LiveExample src='Avatar.main.tsx'>
  <AllExamples.AvatarMainExample client:load />
</LiveExample>

When no profile picture is uploaded, an avatar with the user's initials is shown. It can display one or two letters, or the first letter of their email if no name is provided. You may use an icon instead. Once a photo is uploaded, it replaces the initials/icon.

### Avatar Group

Avatars can be displayed in a list to represent a group of people using `<AvatarGroup>` component.
