...
You can also use `value` and `onValueChange` in `Tabs.Wrapper` to have fully controlled state. You are manually controlling active tab in the example bellow.

```
<Tabs.Wrapper value={myLocalValue} onValueChange={setMyLocalValue}>
  <Tabs.TabList>
    <Tabs.Tab value='apple' label='Apple' key='apple' />
    <Tabs.Tab value='orange' label='Orange' key='orange' />
    <Tabs.Tab value='pear' label='Pear' key='pear' />
...
`Tabs.Wrapper` also has `defaultValue` prop. You can set any tab to be active by default using this.

You can also build `Tabs.Tab` with subcomponents.

```
<Tabs.Tab value='apple' key='apple'>
  <Tabs.TabIcon><SvgAppleIcon /></Tabs.TabIcon>
