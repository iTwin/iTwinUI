...
export type GitHubReleasesData = {
  pkg: PublishablePackages;
  releaseName: string;
  releaseBody: string;
}[];
...
const getGitHubReleaseData = (pkg: PublishablePackages) => {
  ...
  return { pkg, releaseName, releaseBody };
};

const releasePackages = getReleasePackages();
const allGitHubReleasesData: GitHubReleasesData = releasePackages
  .map((pkg) => getGitHubReleaseData(pkg))
  .filter(Boolean) as GitHubReleasesData;

console.log(allGitHubReleasesData);

core.setOutput('github-releases-data', JSON.stringify(allGitHubReleasesData));
