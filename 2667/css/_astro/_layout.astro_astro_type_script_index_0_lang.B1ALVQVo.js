class r extends HTMLElement{#e=!1;constructor(){super();const e=`
    ...
    `;this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
      ...
      ${e}
    `,this.button=this.shadowRoot.querySelector('[aria-label="Page settings"]');const o=window.matchMedia("(prefers-color-scheme: dark)").matches,a=window.matchMedia("(prefers-contrast: more)").matches;this.shadowRoot.querySelector(`input[value=${o?"dark":"light"}${a?"-hc":""}`).checked=!0;const t=document.body,n=o?"dark":"light";t.dataset.iuiTheme=n,t.dataset.iuiContrast=a?"high":void 0,t.classList.toggle("iui-root",!0)}changeTheme=({target:{value:e}})=>{const i=document.body,o=document.documentElement,a=e.endsWith("-hc"),t=a?e.split("-")[0]:e;i.dataset.iuiTheme=t,this.#e&&(o.dataset.colorScheme=t),i.dataset.iuiContrast=a?"high":void 0,this.shadowRoot.querySelector("#theme-color-scheme").innerHTML=`
      :host {
        color-scheme: ${t.includes("light")?"light":"dark"};
      }
    `};changeBackground=({target:{value:e}})=>{e==="bg1"?document.body.style.backgroundColor="var(--iui-color-background)":document.body.removeAttribute("style")};changeBridge=({target:{checked:e}})=>{this.#e=e,document.body.dataset.iuiBridge=e?"true":"false",window.dispatchEvent(new CustomEvent("theme-bridge",{detail:e}))};connectedCallback(){this.shadowRoot.querySelectorAll('input[name="theme"]').forEach(e=>{e.addEventListener("change",this.changeTheme)}),this.shadowRoot.querySelectorAll('input[name="background"]').forEach(e=>{e.addEventListener("change",this.changeBackground)}),this.shadowRoot.querySelector('input[name="theme-bridge"]').addEventListener("change",this.changeBridge)}disconnectedCallback(){this.shadowRoot.querySelectorAll('input[name="theme"]').forEach(e=>{e.removeEventListener("change",this.changeTheme)}),this.shadowRoot.querySelectorAll('input[name="background"]').forEach(e=>{e.removeEventListener("change",this.changeBackground)}),this.shadowRoot.querySelector('input[name="theme-bridge"]').removeEventListener("change",this.changeBridge)}}customElements.define("theme-button",r);
